...
void run(const settings::SettingsNode& settings) {
    ...
    if (runoff_variable.isNull()) {
        throw std::runtime_error(runoff_filename + ": Variable '" + runoff_varname + "' not found");
    }
    ...
    try {
        ...
        throw std::runtime_error(runoff_filename + ": Invalid time variable: " + e.what());
    }
    std::vector<int> times(days_count);
    time_variable.getVar({0}, {days_count}, &times[0]);
    unsigned int year, month, day;
    if (times[0] != 0 || std::sscanf(time_units.c_str(), "days since %u-%u-%u", &year, &month, &day) != 3) {
    ...
    FC_mod_input_irestart = 2;  // just make sure cmaflood thinks it's in spinup mode (i.e. does not read the snapshot)
    ...
    for (std::size_t i = 0; i < spinup_count; ++i) {
